...
bool Sender::sendFile(Message msg)
{
  ...
  std::cout << "--File loc--" <<fileSpec<< std::endl;
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	std::string fileName = msg.file().substr(2,fileName.length());
    std::string fileSpec = saveFilePath + fileName;
	std::cout << "------file Spec:----"<<fileSpec << std::endl;
    std::ofstream saveStream(fileSpec, std::ios::binary);
    if (!saveStream.good())
    {
		std::cout << "------stream no good:----"<< std::endl;
		return false;
    }
    while (true)
    {
      ...
      if (blockSize == 0)
        break;
	  std::cout << "------no break:----" << blockSize << std::endl;
